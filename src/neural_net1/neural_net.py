...
class NeuralNetwork:
    def __init__(self, _x, _y, weights_name, force_random_weights):
        # settings
        ...
        self.init_network(_x, weights_name, force_random_weights)
        self.init_output(_y)

    def init_network(self, _x, saved_weights_name, force_random_weights):
        self.input = _x
        # [[0.  0.  1.]
        #  [0.  1.  1.]
        #  [1.  0.  1.]
    ...
    def feedforward(self):
        ...
        # stop learning if loss is acceptable
        loss = np.mean(np.square(self.y - self.layer2))
        if loss < 0.0001:
            self.learn_rate = 0

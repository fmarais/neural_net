...
def fitness_func(solution, sol_idx):
    ...
    _predictions = pygad.nn.predict(last_layer=GANN_instance.population_networks[sol_idx],
                                    data_inputs=data_inputs)
    correct_predictions = numpy.where(_predictions == data_outputs)[0].size
    _solution_fitness = (correct_predictions / data_outputs.size) * 100

    return _solution_fitness


def callback_generation(_ga_instance):
    global GANN_instance, last_fitness

    population_matrices = pygad.gann.population_as_matrices(
        population_networks=GANN_instance.population_networks,
        population_vectors=_ga_instance.population)

    GANN_instance.update_population_trained_weights(population_trained_weights=population_matrices)

    print("Generation = {generation}".format(generation=_ga_instance.generations_completed))
    print("Fitness    = {fitness}".format(fitness=_ga_instance.best_solution()[1]))
    print("Change     = {change}".format(change=_ga_instance.best_solution()[1] - last_fitness))
    print("---")

    last_fitness = _ga_instance.best_solution()[1].copy()
...
# function holds references to the networks, not their weights. Using such references,
# the weights of all networks can be fetched.
...
# population does not hold the numerical weights of the network instead it holds a list of
# references to each last layer of each network (i.e. solution) in the population. A solution or
# a network can be used interchangeably.
...
# This way is useful when the user wants to start the genetic algorithm with a custom
# initial population.
...
# After the generations complete, some plots are showed
# that summarize how the outputs/fitness values evolve over generations.
